...
var endResultArr = [3, 2, 1];
function stopSpin(result){
    if(result === "win"){
        endResultArr = [1, 1, 1];
        $('#example4 ul').stopSpin();
    }else if(result === "lose"){
        endResultArr = [randomIntFromInterval(1, 4), randomIntFromInterval(1, 4), randomIntFromInterval(1, 4)];
        // resultArr = [2 ,1 ,2];
        if(endResultArr[0] === 1 && endResultArr[1] === 1 && endResultArr[2] === 1){
            endResultArr = [randomIntFromInterval(1, 4), randomIntFromInterval(1, 4), randomIntFromInterval(2, 4)]
        }
        $('#example4 ul').stopSpin();
    }
}
...
function startSpinEnd(result, withSound, _callback){
    ...
    if(result === "win"){
        ...
        resultArr = [randomIntFromInterval(1, 4), randomIntFromInterval(1, 4), randomIntFromInterval(1, 4)];
        // resultArr = [2 ,1 ,2];
        if(resultArr[0] === 1 && resultArr[1] === 1 && resultArr[2] === 1){
            resultArr = [randomIntFromInterval(1, 4), randomIntFromInterval(1, 4), randomIntFromInterval(2, 4)]
        }
    }
    ...
    var winningSlot1 = new Audio('/sound/slots/winningSlot1.mp3');
    var winningSlot2 = new Audio('/sound/slots/winningSlot2.mp3');
    var winningSlot3 = new Audio('/sound/slots/winningSlot3.mp3');
    var losingSlot = new Audio('/sound/slots/losingSlot.mp3');
    ...
    $('#example4 ul').playSpin({
        ...
        onEnd: function() {
            ...
            if(endResultArr[flag] !== 1 || winningState === 0){
                winningState = 0;
                slotSpinState++;

                if(withSound){
                    losingSlot.cloneNode(true).play();
                    // window.losingSlotSound();
                }
            }else{
                if(slotSpinState === 1 && endResultArr[0] === 1 && winningState === 1){
                    slotSpinState++;
                    if(withSound){
                        winningSlot1.play();
                        // window.winningSlot1Sound();
                    }
                }else if(slotSpinState === 2 && endResultArr[1] === 1 && winningState === 1){
                    slotSpinState++;
                    if(withSound){
                        winningSlot2.play();
                        // window.winningSlot2Sound();
                    }
                }else if(slotSpinState === 3 && endResultArr[2] === 1 && winningState === 1){
                    if(withSound){
                        winningSlot3.play();
                        // window.winningSlot3Sound();
                    }
                }
            }
        },
